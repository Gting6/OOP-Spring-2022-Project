...
//			System.out.print(dt.getTime());
			...
			PreparedStatement update_VIP = conn
					.prepareStatement("UPDATE members SET vip_expire_date=? WHERE username=?");
...
//			role stands for his identity
			PreparedStatement stmt = conn
					.prepareStatement("SELECT * FROM " + role + " WHERE username=? AND password=?");
...
//			PreparedStatement stmt = conn.prepareStatement("SELECT * FROM users WHERE username=? AND userpassword=?"); 
			...
			if (res.next()) {
				...
				Member member = new Member(res.getString("username"), res.getString("password"),
						res.getString("address"), res.getString("phone"), res.getString("email"), res.getString("name"),
						res.getDate("vip_expire_date")); // need recreate or just send these 2 as json file?
				conn.close();
				return member;
			} else {
...
//				System.out.println(res.getString("name"));
	...
	public ArrayList<String[]> searchRestaurantByName(String name) throws SQLException {
		try {
			...
			PreparedStatement stmt = conn
					.prepareStatement("SELECT * from restaurants WHERE (lower(name) LIKE '%" + name + "%')");
	...
	public ArrayList<String[]> searchRestaurantByType(String type) throws SQLException {
		try {
			...
			PreparedStatement stmt = conn.prepareStatement(
					"select * from restaurants where username in (select restaurant_name from type_restaurants WHERE type = \""
							+ type + "\")");
...
//			role stands for his identity
			...
			if (res.next()) {
				...
				Order order = new Order(res.getString("id"), res.getInt("status"), d, deliver_time, d_bar,
						res.getString("member_name"), res.getString("deliveryman_name"),
						res.getString("restaurant_name"), res.getInt("fee"));
				conn.close();
				return order;
//				might add cookie or something else??
			} else {
...
//			role stands for his identity
	...
	public ArrayList<String> getOrdersRestaurant(String restaurant_name) throws SQLException {
		...
		try {
			...
			PreparedStatement query_orders = conn
					.prepareStatement("SELECT * from orders WHERE restaurant_name=? order by create_time");
	...
	public ArrayList<String> getOrdersNoDeliveryman() throws SQLException {
		...
		try {
			...
			PreparedStatement query_orders = conn.prepareStatement(
					"SELECT * from orders WHERE deliveryman_name=? and status=2 order by create_time");
	...
	public ArrayList<String> getOrdersDeliveryman(String Deliveryman_id) throws SQLException {
		...
		try {
			...
			PreparedStatement query_orders = conn
					.prepareStatement("SELECT * from orders WHERE deliveryman_name=? order by create_time");
	...
	public ArrayList<String> getOrdersMember(String member_id) throws SQLException {
		...
		try {
			...
			PreparedStatement query_orders = conn
					.prepareStatement("SELECT * from orders WHERE member_name=? order by create_time");
